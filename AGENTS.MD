...
`Arb.NET.Analyzers` - Analyzer for quick fixes related to missing ARB keys in the code.
...
`Arb.NET.IDE.Common` - shared code for the IDE plugins
...
## dotnet tool
- The `Arb.NET.Tool` project can be installed as a dotnet tool.

## Scripts
- There are various `publish-*.ps1` scripts. Use them to validate your changes.
...
## Rules
- Any file IO should be done with `Constants.UTF8_NO_BOM` encoding to avoid issues with byte order marks in generated files.

### Tips

For disassembly of .NET code you can use `ilspycmd`. If not installed, request it if you think it would be useful. 
